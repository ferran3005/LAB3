...
import eu.su.mas.dedaleEtu.sid.grupo06.bdi.goals.ComputeNextPositionGoal;
import eu.su.mas.dedaleEtu.sid.grupo06.bdi.goals.ComputeRandomMovementGoal;
import eu.su.mas.dedaleEtu.sid.grupo06.bdi.goals.SendMovementRequestGoal;
import eu.su.mas.dedaleEtu.sid.grupo06.bdi.goals.SendUpdateRequestGoal;
import eu.su.mas.dedaleEtu.sid.grupo06.bdi.goals.ShoutOntologyGoal;
import eu.su.mas.dedaleEtu.sid.grupo06.common.MovementData;
...
public class KeepMailboxEmptyPlanBody extends AbstractPlanBody {  //TODO: MUCHO de lo que hay aquí se puede extraer em handlers (i.e. sparql queries, message handling, goals, etc.)
    ...
    @Override
    public void action() {
        if(message.getSender().getName().equals(((BDIAgent06) getCapability().getMyAgent()).situatedAgent.getName())) {
    ...
    private void handleDataResponses(ACLMessage message) {
        ((BDIAgent06) getCapability().getMyAgent()).situatedData = new Gson().fromJson(message.getContent(), SituatedData.class);
        ((BDIAgent06) getCapability().getMyAgent()).situatedData.setSituatedAgent(((BDIAgent06) getCapability().getMyAgent()).situatedAgent);
    ...
    private void updateOntologyWithObservations(String json) {
        ...
        String situatedAgentName = ((BDIAgent06) getCapability().getMyAgent()).situatedData.getSituatedAgent().getLocalName();
        Model model = (Model) getCapability().getBeliefBase().getBelief(ONTOLOGY).getValue();
    ...
    private void handleMovementResponses(ACLMessage message) {
        if (message.getPerformative() == ACLMessage.AGREE) {
            ...
            ((BDIAgent06) getCapability().getMyAgent()).routeHandler.discardTop(); //TODO: creo que esto ya no nos vale
            getCapability().getBeliefBase().updateBelief(AGENT_STATE, BdiStates.INITIAL);
            addRequestUpdateGoal();
            ...
            ((BDIAgent06) getCapability().getMyAgent()).situatedData.setBackPackCapacityDiamond(mov.backpackFreeSpaceDiamate);
            ((BDIAgent06) getCapability().getMyAgent()).situatedData.setBackPackCapacityGold(mov.backpackFreeSpaceOro);
            ((BDIAgent06) getCapability().getMyAgent()).situatedData.setTreasureType(mov.treasureType); //treasure type se asigna a lo primero que recogen

            String situatedAgentName = ((BDIAgent06) getCapability().getMyAgent()).situatedData.getSituatedAgent().getLocalName();
            OntologyManager.addCurrentPosition(situatedAgentName, currentPosition, model);
            ((BDIAgent06) getCapability().getMyAgent()).routeHandler.updateAfterMovement();
